...
class MotionGraph:
    def __init__(self, lib, n_neighbors=16, src_stride=2, tgt_stride=2, pca_dim=16,
        ...
        cpath = os.path.join(C.ROOT, "data", "motion_graph.pkl")
        ...
        # PCA-reduce the 62-d descriptor so KDTree queries are not near-brute-force.
        desc = _descriptors(lib)
        X = desc - desc.mean(0)
        _, _, Vt = np.linalg.svd(X, full_matrices=False)
        P = (X @ Vt[:pca_dim].T).astype(np.float32)
    ...
    def follow_command(self, command, seconds, start_frame=0, return_trace=False):
        ...
        while step < n:
            ...
            if decide:
                want = command.desired_velocity(step * C.DT)        # world frame
                c, s = np.cos(-cwy), np.sin(-cwy)                    # express in robot-local frame
                want_local = np.array([c * want[0] - s * want[1], s * want[0] + c * want[1]])
                ...
                if src >= 0:
                    for j, dist in self.trans[src]:
                        cand.append((j, 0.1 + 0.05 * dist, True))   # small switch cost + blend cost
                ...
                for f, pen, is_tr in cand:
                    cost = np.linalg.norm(self._seg_vel_local(f) - want_local) + pen
                    if cost < best_cost:
