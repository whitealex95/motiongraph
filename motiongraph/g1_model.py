...
class G1Model:
    ...
    def _build_mirror_map(self):
        """For each hinge dof, the qpos index it maps to under a sagittal (y->-y) reflection
        and its sign. Rule: swap left<->right joints; rotation about pitch (y) keeps sign,
        roll (x)/yaw (z) negate. Built from the model so it tracks any joint reordering."""
        m = self.model
